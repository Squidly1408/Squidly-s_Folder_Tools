import os
from tkinter import Tk, filedialog, messagebox, Toplevel, Label, Button, Frame, StringVar, OptionMenu
...
def themed_error_message(error_message):
    ...
    error_label = Label(error_frame, text=error_message, bg="#f2f2f2", font=("Arial", 12), wraplength=380)
    error_label.pack(pady=20)

    ok_button = Button(error_frame, text="OK", command=error_window.destroy, bg="#007BFF", fg="white", font=("Arial", 10))
...
def convert_files(source_files, destination_format):
    ...
    source_format = os.path.splitext(source_files[0])[1][1:]  # Get the file extension without the dot
    converters = {
        'txt': {
        ...
        'jpg': {
            'png': convert_jpg_to_png,
            'gif': convert_jpg_to_gif
        },
        'png': {
            'jpg': convert_png_to_jpg,
            'gif': convert_png_to_gif
        },
        'wav': {
            'mp3': convert_wav_to_mp3,
            'ogg': convert_wav_to_ogg
        },
        'mp3': {
            'wav': convert_mp3_to_wav,
            'ogg': convert_mp3_to_ogg
        },
        'xlsx': {
    ...
    if source_format not in converters or destination_format not in converters[source_format]:
        themed_error_message(f"Conversion from {source_format} to {destination_format} is not supported.")
        return
    ...
    for source_file in source_files:
        ...
        try:
            ...
            themed_error_message(f"Error converting {source_file} to {destination_format}: {str(e)}")

    messagebox.showinfo("Conversion complete", "All selected files have been converted.")
...
def select_files():
    ...
    format_label = Label(format_window, text="Choose destination format:", font=("Arial", 12))
    format_label.pack(pady=10)

    format_options = ["pdf", "txt", "docx", "xlsx", "png", "jpg", "gif", "mp3", "wav", "ogg", "csv"]
    format_menu = OptionMenu(format_window, format_var, *format_options)
    format_menu.pack(pady=10)

    convert_button = Button(format_window, text="Convert", command=lambda: convert_files(source_files, format_var.get()))

...
def rename_files(substring_to_remove, folder_path):
    ...
    for file in commands_path.glob("*.py"):
        ...
        if substring_to_remove in file_name:
            ...
            new_file_path = commands_path / f"{new_file_name}.py"  # Create new file path
...
def on_rename():
    ...
    if renamed_files:
        messagebox.showinfo("Renaming Complete", f"Renamed {len(renamed_files)} file(s):\n" +
...
# Rename button
rename_button = tk.Button(root, text="Rename Files", command=on_rename, state="disabled")

...
def solve_multiplication():
    try:
        ...
        explanation_text.insert(
            tk.END,
            f"Explanation: To solve this, we multiply {num1} and {num2} together.\n\nPython Example:\nnum1 = {num1}\nnum2 = {num2}\nresult = num1 * num2\n",
        )
...
def create_ui(root):
    ...
    solve_button = tk.Button(
        root, text="Solve Multiplication", command=solve_multiplication
    )
    solve_button.pack()
...
def run_gui():
    root = tk.Tk()
    root.title("Multiplication Solver")
    create_ui(root)
    root.mainloop()

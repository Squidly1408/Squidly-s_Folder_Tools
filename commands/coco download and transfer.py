...
class DatasetDownloader:
    def __init__(self, root):
        ...
        self.class_frame = tk.LabelFrame(self.scrollable_frame, text="Select Classes", padx=10, pady=10)
        ...
        self.format_dropdown = ttk.Combobox(self.scrollable_frame, textvariable=self.format_var, values=SUPPORTED_FORMATS, state='readonly')
        self.format_dropdown.pack(pady=5)

        # Choose output location button
        self.save_location = tk.StringVar()
        save_button = tk.Button(self.scrollable_frame, text="Choose Save Location", command=self.choose_save_location)
        save_button.pack(pady=10)

        # Display chosen save location
        self.save_location_label = tk.Label(self.scrollable_frame, text="No location chosen", fg="red")
        self.save_location_label.pack()

        # Create progress bar
        self.progress_bar = ttk.Progressbar(self.scrollable_frame, orient="horizontal", length=300, mode="determinate")
        self.progress_bar.pack(pady=10)

        # Download button
        download_button = tk.Button(self.scrollable_frame, text="Download Dataset", command=self.download_dataset)
    ...
    def download_dataset(self):
        ...
        if not selected_classes:
            messagebox.showwarning("No Classes Selected", "Please select at least one class.")
            return
        ...
        messagebox.showinfo("Downloading", f"Downloading and processing dataset for classes: {', '.join(selected_classes)} in format {selected_format}")
        ...
        self.save_filtered_dataset(filtered_annotations, selected_classes, Path(save_location) / selected_format)

        self.progress_bar["value"] = 100
        self.root.update_idletasks()
        
        messagebox.showinfo("Completed", f"Dataset prepared successfully in format: {selected_format} at {save_location}")
    ...
    def filter_annotations(self, selected_classes):
        ...
        class_ids = {category["id"]: category["name"] for category in coco_data["categories"] if category["name"] in selected_classes}
        ...
        filtered_annotations["categories"] = [cat for cat in coco_data["categories"] if cat["name"] in selected_classes]
